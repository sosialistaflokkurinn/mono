...
import { BeakerIcon, Cog6ToothIcon, HomeIcon, UserIcon } from "@heroicons/react/16/solid";
...
const Sidebar = ({ children, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
	<div className="flex w-64 flex-col border-gray-200 border-r bg-white" {...props}>
		{children}
	</div>
);

const SidebarHeader = ({ children, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
	<div className="border-gray-200 border-b p-4" {...props}>
		{children}
	</div>
);

const SidebarBody = ({ children, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
...
const SidebarFooter = ({ children, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
	<div className="mt-auto border-gray-200 border-t p-4" {...props}>
		{children}
	</div>
);

const SidebarHeading = ({ children, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
	<h3 className="px-3 py-2 font-medium text-gray-500 text-sm uppercase tracking-wider" {...props}>
		{children}
	</h3>
);
...
const SidebarItem = ({
	...
	const baseClassName = "px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded-md mx-2";
...
const SidebarSection = ({ children, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
