    ...
    <NavbarContext value={contextValue}>
      <header
        ...
        className={twMerge(
          ...
          intent === "inset" && "bg-navbar dark:bg-bg min-h-svh",
          className,
        )}
        {...props}
      >
  ...
  return (
    <Link
      ...
      className={composeTailwindRenderProps(
        className,
        twJoin(
          "text-muted-fg outline-hidden relative flex cursor-pointer items-center gap-x-2 px-2 no-underline transition-colors *:data-[slot=icon]:-mx-0.5 md:text-sm forced-colors:transform-none forced-colors:outline-0 forced-colors:disabled:text-[GrayText]",
          "pressed:text-fg hover:text-fg focus:text-fg focus-visible:outline-primary focus-visible:outline-1",
          ...
          isCurrent && "text-navbar-fg cursor-default",
        ),
      )}
      {...props}
    >
...
const NavbarLogo = ({ className, ...props }: LinkProps) => {
  return (
    <Link
      className={composeTailwindRenderProps(
        className,
        "text-fg focus:outline-hidden focus-visible:outline-primary relative flex items-center gap-x-2 px-2 py-4 focus-visible:outline-1 md:mr-4 md:px-0 md:py-0",
      )}
      {...props}
    />
  );
...
const NavbarCompact = ({ className, ref, ...props }: NavbarCompactProps) => {
  ...
  return (
    <div
      ref={ref}
      className={twMerge(
        "bg-navbar text-navbar-fg flex justify-between peer-has-[[data-navbar-intent=floating]]:border md:hidden",
