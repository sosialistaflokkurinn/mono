...
const Navbar = ({ children, ...props }: React.HTMLAttributes<HTMLElement>) => (
	<nav className="flex items-center border-gray-200 border-b bg-white px-4 py-3" {...props}>
		{children}
	</nav>
);

const NavbarSection = ({ children, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
...
const NavbarItem = ({ children, href, className, onClick }: NavbarItemProps) => {
...
const Dropdown = ({ children, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
	<div className="relative inline-block text-left" {...props}>
		{children}
	</div>
);

const DropdownButton = ({ children, ...props }: React.HTMLAttributes<HTMLButtonElement>) => (
	<button
		className="inline-flex items-center justify-center rounded-md bg-white px-3 py-2 font-medium text-gray-900 text-sm shadow-sm ring-1 ring-gray-300 ring-inset hover:bg-gray-50"
		{...props}
	>
		{children}
	</button>
...
const DropdownItem = ({ children, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
	<div className="block px-4 py-2 text-gray-700 text-sm hover:bg-gray-100" {...props}>
		{children}
	</div>
);
